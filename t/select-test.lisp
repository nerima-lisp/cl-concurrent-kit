  ...
  (let ((channel (make-channel :buffer-size 1))
    ...
    (unwind-protect
        (progn
          ...
          (expect (wait-on-semaphore recv-waiter :timeout 0.0d0) :to-be-truthy)
          (expect (wait-on-semaphore both-waiter :timeout 0.0d0) :to-be-truthy)
          (expect (wait-on-semaphore send-waiter :timeout 0.0d0) :to-be nil)
          (try-recv channel)
          (expect (wait-on-semaphore send-waiter :timeout 0.0d0) :to-be-truthy)
          (expect (wait-on-semaphore both-waiter :timeout 0.0d0) :to-be-truthy))
