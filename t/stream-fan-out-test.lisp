  ...
  (it "keeps draining INPUT to the remaining outputs once one is closed"
    (let ((input (make-channel :buffer-size 3)))
      ...
      (multiple-value-bind (outputs completion) (channel-broadcast input 2 :buffer-size 3)
        ...
        (expect (drain-channel (second outputs) :timeout +test-timeout+)
                :to-equal (list 1 2 3))
        (await completion :timeout +test-timeout+))))
    ...
    (let ((input (make-channel :buffer-size 3)))
      ...
      (with-task-scope (scope)
        (multiple-value-bind (outputs completion) (channel-broadcast input 1 :buffer-size 3 :scope scope)
          (expect (drain-channel (first outputs) :timeout +test-timeout+)
                  :to-equal (list 1 2 3))
          (await completion :timeout +test-timeout+))))))
    ...
    (let ((input (make-channel :buffer-size 4)))
      ...
      (multiple-value-bind (output completion) (channel-take 2 input)
        (expect (drain-channel output :timeout +test-timeout+)
                :to-equal (list 1 2))
        (await completion :timeout +test-timeout+)
        (expect (recv input :timeout +test-timeout+) :to-be 3))))
  ...
  (it "forwards the matching prefix and consumes (without forwarding) the first non-match"
    (let ((input (make-channel :buffer-size 5)))
      ...
      (multiple-value-bind (output completion) (channel-take-while (function oddp) input)
        ...
        ;; remains.
        (expect (recv input :timeout +test-timeout+) :to-be 7))))
  ...
  (it-property "reassembles into the original list for any SIZE, with every batch but the last exactly SIZE long"
    ...
    (let ((input (make-channel :buffer-size (max 1 (length values)))))
      ...
      (multiple-value-bind (output completion) (channel-batch size input)
        (let ((batches (drain-channel output :timeout +test-timeout+)))
