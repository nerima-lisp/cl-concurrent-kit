;;;; src/conditions.lisp
(progn
  (declaim (optimize #+sb-cover (sb-c:store-coverage-data 3)))
  (in-package #:cl-concurrent-kit))

(define-condition cl-concurrent-kit-error (error)
  ()
  (:documentation
    "Base condition for every error CL-CONCURRENT-KIT signals.
...
(define-condition operation-timed-out (cl-concurrent-kit-error)
  ((operation
      ...
      :documentation
      "A keyword naming the operation that timed out, e.g. :AWAIT, :RECV, or :SEND.")
    (timeout
      :initarg
      :timeout
      :reader
  ...
  (:documentation
    "Signaled by AWAIT, RECV, or SEND when their :TIMEOUT keyword argument elapses before the operation completes."))
...
(define-condition promise-already-fulfilled (cl-concurrent-kit-error)
  ((promise
      ...
      :documentation
      "The promise DELIVER or DELIVER-ERROR was called on a second time."))
  (:report
    (lambda (condition stream)
      (format
        stream
  ...
  (:documentation
    "Signaled by DELIVER or DELIVER-ERROR when called on a
...
(define-condition channel-closed (cl-concurrent-kit-error)
  ((channel
      ...
      :documentation
      "The channel SEND or TRY-SEND was called on after it was closed."))
  (:report
    (lambda (condition stream)
      (format
        stream
        "Cannot send on closed channel ~S."
        (channel-closed-channel condition))))
  (:documentation
    "Signaled by SEND or TRY-SEND on a channel that CLOSE-CHANNEL
...
(progn
  (define-condition executor-shut-down (cl-concurrent-kit-error)
    ((executor
        :initarg
        :executor
        :reader
        ...
        :documentation
        "The executor that rejected or cancelled the task."))
    (:report
      (lambda (condition stream)
        (format
          stream
          "Executor ~S has shut down."
          (executor-shut-down-executor condition))))
    (:documentation
      "Delivered through a submitted promise when its executor no longer accepts work."))
  (define-condition task-cancelled (cl-concurrent-kit-error)
    ((scope
        ...
        :documentation
        "The scope whose cancellation CHECK-CANCELLED observed."))
    (:report
      (lambda (condition stream)
        (format
          stream
    ...
    (:documentation
      "Signaled by CHECK-CANCELLED when called from within a task
...
(define-condition scope-error (cl-concurrent-kit-error)
  ((causes
      ...
      scope-error-causes
      :documentation
      "A list of the conditions signaled by the scope's failed child
tasks, oldest first."))
  (:report
    (lambda (condition stream)
      (let ((causes (scope-error-causes condition)))
        (format
          ...
          (first causes)))))
  (:documentation
    "Signaled by WITH-TASK-SCOPE when its body completes
