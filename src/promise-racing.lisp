...
NIL."
  (let* ((promises (%check-promises (coerce promises 'vector)))
...
(defun promise-timeout (promise timeout)
  "Return a PROMISE that mirrors PROMISE unless TIMEOUT (a
CL-DATE-KIT:DURATION) elapses first, in which case it fails with
OPERATION-TIMED-OUT and stops observing PROMISE. Spawns one thread to run the
timer; whichever side loses the race -- PROMISE settling first, or the timer
firing first -- stops the other promptly instead of leaking an observer or a
sleeping thread until it happens to finish on its own."
  (check-type promise promise)
  (check-type timeout cl-date-kit:duration)
  (setf timeout (cl-date-kit:duration-to-seconds timeout))
