...
(defmacro with-timeout (seconds &body body)
  "Run BODY under a preemptive deadline of SECONDS seconds, returning its
values. If BODY has not finished by then it is interrupted and
OPERATION-TIMED-OUT is signaled, naming :WITH-TIMEOUT as the operation.

SECONDS NIL, zero, or negative means no deadline at all: BODY simply runs.
Zero and negative match SB-EXT:WITH-TIMEOUT, which schedules no timer unless
its argument is strictly positive -- a zero-second deadline that fired
immediately would leave (WITH-TIMEOUT 0 ...) unable to run anything. NIL is
this macro's own addition, so a caller holding a timeout variable that is NIL
for \"no limit\" can pass it straight through, exactly as AWAIT, RECV, SEND
and SELECT already accept a NIL :TIMEOUT; SB-EXT:WITH-TIMEOUT itself signals a
TYPE-ERROR on NIL.
